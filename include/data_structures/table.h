...
public:
  ...
  struct TableEntry
  {
    TableEntry(const SymbolType& s, const FrequencyType& f, const BinaryNumber& n) : symbol(s),
                                                                                     frequency(f),
                                                                                     binary_number(n) {}
...
public:
   ...
   *
   */
  constexpr Table(std::initializer_list<EntryType>&& v) noexcept : symbols_(std::move(v)) {}
  /*!
   * \brief Table Creates a table with the passed data.
   * \param data The new data of the table
   */
  template <typename T>
  explicit constexpr Table(const T& data) : symbols_(data) {}
  /*!
   * \brief Table Creates a table with the passed data.
   *
   ...
   */
  template <typename T>
  explicit constexpr Table(T&& data) noexcept : symbols_(std::move(data)) {}
  /*!
